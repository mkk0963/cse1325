...
public class ThreadsOfSudoku 
{
    ...
    public static void main(String[] args) 
    {
        try 
        {
            ...
            int sudsPerThread = (int) Math.ceil((double) suds.length / numThreads);

            for (int i = 0; i < numThreads; ++i) 
            {
                final int start = i * sudsPerThread;
                final int end = Math.min((i + 1) * sudsPerThread - 1, suds.length - 1);
                final int id = i + 1;

                Thread thread = new Thread(() -> solveSuds(start, end, id));
                threads.add(thread);
            }
